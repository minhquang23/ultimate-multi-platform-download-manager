...
def fetch_video_list(urls, browser='chrome'):
    """
            ...
            try:
                if platform in ["youtube", "youtube_shorts"]:
                    ...
                    for entry in entries:
                        ...
                        results.append({'url': v_url, 'title': title, 'is_main': is_main, 'id': v_id, 'platform': platform})
                    ...
                    with yt_dlp.YoutubeDL(ydl_fast_opts) as ydl_fast:
                        ...
                        results.append({'url': url, 'title': title, 'is_main': True, 'id': v_id, 'platform': platform})

            except Exception as e:
                print(f"Lỗi quét URL {url}: {e}")
                results.append({
                    'url': url,
...
def download_subtitles_for_url(url, lang='vi', output_dir='downloads', download_video=True,
    ...
    """
    Tải video và tạo transcript cho 1 URL lẻ.

    Luồng xử lý:
    - YouTube (prefer_subtitle): Tải .vtt → Làm sạch → .txt
    - YouTube (always_whisper): Tải video → Whisper → .txt
    - TikTok / Facebook / Vimeo / ...: Tải video → Whisper → .txt
    """
    ...
    use_subtitle = is_youtube and transcript_mode == 'prefer_subtitle'
    use_whisper = not use_subtitle  # Whisper cho TikTok/Reels HOẶC khi always_whisper
    ...
    try:
        ...
        elif use_whisper and download_video:
        ...
        if log_callback:
            log_callback(f"❌ Lỗi khi xử lý {url}: {e}")
        return False, 0
...
def process_multiple_urls(urls, lang='vi', output_dir='downloads', download_video=True,
                          browser='chrome', delay_min=3.0, delay_max=5.0,
                          video_quality='1080p', transcript_mode='prefer_subtitle',
                          whisper_model='medium', whisper_device='auto',
                          whisper_model_dir='models/whisper',
                          enable_speaker_diarization=False, gemini_api_key='',
                          progress_callback=None, delay_callback=None, log_callback=None, diarization_progress_callback=None):
    """
    Xử lý danh sách URL tuần tự với khoảng nghỉ chống quét bot (chính xác milisecond).
    """
    if log_callback:
        mode_text = "Video + Transcript" if download_video else "Chỉ Transcript"
        log_callback(f"🚀 Bắt đầu [{mode_text}] — {len(urls)} video(s) | Thư mục: {output_dir}")
    ...
    if log_callback:
        log_callback(f"\n{'='*50}")
        log_callback(f"✅ HOÀN THÀNH: {success_count}/{len(urls)} video thành công | Thời gian: {duration:.1f}s")
        log_callback(f"{'='*50}")
